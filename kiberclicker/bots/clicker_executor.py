...
class ClickerExecutor(OpenAIExecutor):
    ...
    async def init_browser(self, web_link: str):
        if self.browser is None:
            ...
            token = os.environ.get('COOKIE_TOKEN', 'aaa')
            domain = os.environ.get('COOKIE_DOMAIN', 'example.com')
            ...
            await self._init_page(web_link)

    async def _init_page(self, web_link: str):
        ...
        await self.page.wait_for_timeout(timeout=2600)
    ...
    async def get_click_options(self, screenshot_bytes: bytes, request: str | None = None, amount=5):
        ...
        return await self._analyse_screenshot(screenshot_bytes,
                                              ...
                                              f"We already checked this ones {self.clicked}, so do not duplicate and find new click targets.",
                                              amount=amount)

    async def _analyse_screenshot(self, screenshot_bytes: bytes, request: str = None, amount=5) -> dict:
    ...
    async def click(self, page: Page, point: ClickParameters, open_new=False):
        ...
        await new_page.click(f"text={point.exact_element_text}", timeout=1300)
        ...
        self.clicked.append(point.exact_element_text)
        await new_page.close()
        return result
